...
public:
	...
	bool forwardCheck2point0(std::vector<Variable*> &s);
	...
	struct OrderByDomain
	{
		bool operator() (std::pair<int, Variable* >left, std::pair<int, Variable* > right) { return (left.first > right.first); };
	};
	struct OrderByConstraint
	{
		bool operator() (std::pair<int, int>left, std::pair<int, int > right) { return (left.first > right.first); };
	};
	struct OrderByMostNeighbors
	{
		bool operator() (std::pair<int, Variable* >left, std::pair<int, Variable* > right) { return (left.first <= right.first); };
	};
	struct OrderByMAD
	{
		bool operator() (std::pair<Variable*, std::pair<int, int>>left, std::pair<Variable*, std::pair<int, int>>right)
		{
		}
	};
private:
